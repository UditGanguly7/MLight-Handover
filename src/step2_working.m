%% STEP 2: MLight-Handover - Working with Your Data
clear; close all; clc;
...
%% Fix elevation: Use absolute value or detect based on Doppler
% Your elevation is negative, but we can use Doppler magnitude as proxy
[num_steps, num_ues, num_sats] = size(doppler_full);

% Find visible satellites based on Doppler magnitude
% When satellite is visible, |Doppler| is large (> 30 kHz)
doppler_mag = abs(doppler_full);
visible_threshold = 30000;  % 30 kHz threshold

% Find time indices where any satellite has strong Doppler
visible_mask = squeeze(doppler_mag(:, 1, :) > visible_threshold);
...
if ue_found == -1
    error('No visible satellites found. Check Doppler values.');
end
...
% Create synthetic elevation based on Doppler profile
% Doppler is maximum at horizon, zero at zenith
% So we can estimate elevation from Doppler
max_doppler_abs = max(abs(doppler_ue));
normalized_doppler = abs(doppler_ue) / max_doppler_abs;
synthetic_elev = 90 * (1 - normalized_doppler);  % Rough approximation
...
time_remaining = (length(vis_indices) - mid_pos) * 1 / 60;  % assuming 1 sec steps
...
for i = 1:num_visible
    ...
    % Estimation error (increases with lower Doppler)
...
subplot(2,2,3);
plot((1:num_visible)/60, abs(doppler_ue(vis_indices))/1000, 'm-', 'LineWidth', 1);
xlabel('Time (minutes)'); ylabel('|Doppler| (kHz)');
title('Doppler Magnitude (Visibility Proxy)');
yline(30, 'r--', 'Visibility Threshold');
grid on;
...
sgtitle('MLight-Handover: Step 2 - PRACH Residuals');
...
save('step2_features.mat', 'feature_vector', 'timing_residuals', ...
     'doppler_residuals', 'vis_indices');

fprintf('\nSaved: step2_features.mat\n');
fprintf('\n========================================\n');
fprintf('STEP 2 COMPLETE - Ready for DQN (Step 3)\n');
fprintf('========================================\n');
...
fprintf('  Ready for handover decision (78.2%% target)\n');
